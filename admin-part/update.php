	...
	if ( isset($_POST['submit'])) {
		...
		if ($valid) {
			if(is_uploaded_file($_FILES['fileName']['tmp_name'])){
		            ...
		            {
						$query = "UPDATE `products` set `name`= '$name',`description`= '$description',`price`='$price',`image`='$image',`star`='$star' WHERE id ='$id'";
						echo "$query";
						$con->query($query);
						header("location: retrieve.php");
					}
...
<body>
	<div class="main">
		...
		<div class="span10 offset1">
		    <form class="form-signin" method="post" action="update.php?id=<?php echo $id?>" enctype="multipart/form-data" >       
		    	<h2 class="form-signin-heading" style="color:white"><strong>Update</strong></h2>
	        	...
	        	<input type="text" id="price" name="price" placeholder="Price" class="form-control" value="<?php echo !empty($price)?$price:'';?>">
					...
					<?php endif;?>
				<input type="number" id="star" max="5" min="3" name="star" placeholder="Star" class="form-control" value="<?php echo !empty($star)?$star:'';?>">
				...
				<div style="position:relative;">
			        <a class='btn btn-primary' href='javascript:;'>
			            Choose File...
			            <input type="file" value="<?php echo !empty($image)?$image:'';?>" enctype="multipart/form-data" style='position:absolute;z-index:2;top:0;left:0;filter: alpha(opacity=0);-ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)";opacity:0;background-color:transparent;color:transparent;' name="fileName" size="40"  value="<?php echo !empty($image)?$image:'';?>" onchange='$("#upload-file-info").html($(this).val());'>
			        </a>
			    ...
			    <button type="submit" class="btn btn-success up" name='submit'><strong>Update</strong></button>
